...
*
* Copyright (C) 2015-2026 by Sascha Willems (www.saschawillems.de)
*
...
#undef STR
	...
	inline QString formatFeature2String(const VkFormatFeatureFlagBits2 feature)
	{
		switch (feature)
		{
#define STR(r) case VK_FORMAT_FEATURE_2_##r: return #r
			STR(SAMPLED_IMAGE_BIT);
			STR(STORAGE_IMAGE_BIT);
			STR(STORAGE_IMAGE_ATOMIC_BIT);
			STR(UNIFORM_TEXEL_BUFFER_BIT);
			STR(STORAGE_TEXEL_BUFFER_BIT);
			STR(STORAGE_TEXEL_BUFFER_ATOMIC_BIT);
			STR(VERTEX_BUFFER_BIT);
			STR(COLOR_ATTACHMENT_BIT);
			STR(COLOR_ATTACHMENT_BLEND_BIT);
			STR(DEPTH_STENCIL_ATTACHMENT_BIT);
			STR(BLIT_SRC_BIT);
			STR(BLIT_DST_BIT);
			STR(SAMPLED_IMAGE_FILTER_LINEAR_BIT);
			STR(SAMPLED_IMAGE_FILTER_CUBIC_BIT);
			STR(TRANSFER_SRC_BIT);
			STR(TRANSFER_DST_BIT);
			STR(SAMPLED_IMAGE_FILTER_MINMAX_BIT);
			STR(MIDPOINT_CHROMA_SAMPLES_BIT);
			STR(SAMPLED_IMAGE_YCBCR_CONVERSION_LINEAR_FILTER_BIT);
			STR(SAMPLED_IMAGE_YCBCR_CONVERSION_SEPARATE_RECONSTRUCTION_FILTER_BIT);
			...
			STR(DISJOINT_BIT);
			STR(COSITED_CHROMA_SAMPLES_BIT);
			STR(STORAGE_READ_WITHOUT_FORMAT_BIT);
			STR(STORAGE_WRITE_WITHOUT_FORMAT_BIT);
			STR(SAMPLED_IMAGE_DEPTH_COMPARISON_BIT);
			STR(VIDEO_DECODE_OUTPUT_BIT_KHR);
			STR(VIDEO_DECODE_DPB_BIT_KHR);
			STR(ACCELERATION_STRUCTURE_VERTEX_BUFFER_BIT_KHR);
			STR(FRAGMENT_DENSITY_MAP_BIT_EXT);
			STR(FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR);
			STR(HOST_IMAGE_TRANSFER_BIT_EXT);
			STR(LINEAR_COLOR_ATTACHMENT_BIT_NV);
			STR(WEIGHT_IMAGE_BIT_QCOM);
			STR(WEIGHT_SAMPLED_IMAGE_BIT_QCOM);
			STR(BLOCK_MATCHING_BIT_QCOM);
			STR(BOX_FILTER_SAMPLED_BIT_QCOM);
			STR(OPTICAL_FLOW_IMAGE_BIT_NV);
			STR(OPTICAL_FLOW_VECTOR_BIT_NV);
			STR(OPTICAL_FLOW_COST_BIT_NV);
			STR(VIDEO_ENCODE_INPUT_BIT_KHR);
			STR(VIDEO_ENCODE_DPB_BIT_KHR);

#undef STR
		default: return QString::fromStdString("UNKNOWN_ENUM (" + toHexString(feature) + ")");
		}
	}
